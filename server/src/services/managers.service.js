...
function list({ page, limit }) {
  ...
  const items = db.prepare(`
    SELECT am.id, am.user_id, u.name, u.email, am.created_by, am.created_at
...
function create(userId, adminId) {
  ...
  db.prepare("UPDATE users SET role = 'admin', updated_at = datetime('now','localtime') WHERE id = ?").run(userId);
  logger.info(`Admin created: userId=${userId}, by=${adminId}`);
